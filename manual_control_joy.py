...
class KeyboardControl(object):
    ...
    def _parse_vehicle_joy(self,world):
        ...
        axis_tb = joystick.get_axis(3)
        if(axis_tb<0.007781982421875):
            self._control.throttle = min(-1*axis_tb, 0.46)
            #self._control.throttle = min(-1*axis_tb, 1)
        elif(axis_tb>0.007781982421875):
            self._control.brake = min(axis_tb, 1)
        else:
            self._control.throttle = 0.0
            self._control.brake = 0

        self._control.steer = axis_lr/4
